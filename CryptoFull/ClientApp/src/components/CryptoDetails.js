...
const CryptoDetails = ({
  ...
  const cryptoURL = `/api/cryptodetail/${cryptoId}`;
  const cryptoDataURL = `/api/cryptograph/${cryptoId}`;
  let dates = [];
  let prices = [];
  let hasData = false;
  ...
  const changeFavorite = () => {
    ...
    if (favorite) {
      axios.delete(`/api/favorite/${crypto.id}/${user.id}`);
    } else {
      axios.post(`/api/favorite/${user.id}`, selectedCrypto);
    }
  };
